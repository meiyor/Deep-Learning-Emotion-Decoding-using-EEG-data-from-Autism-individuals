...
strn2=strn1[8].split('.') ## change it for 10 or 9 depending if the folder name has a _ or you can set it up your absolute data path   

csvFile1 = str(sys.argv[1]) # triaining and test files taking per subject specifiy these files in the LOTO cross-validation in an upper level
...
os.remove(cwd+'/file_report.txt') ## remove the previous trial report file called file_report.txt
...
num_colst = test_data_file.shape[0] #test_data_file.shape[1] # Length of Data Vector each column can be assumed as a flattened feature in the file

num_rowsm = method_file.shape[0] # Number of data samples                                                                                                                         
num_colsm = method_file.shape[1] #test_data_file.shape[1] # Length of Data Vector each column can be assumed as a flattened feature in the file
...
data_tr=data_train.reshape((-1,752,30,1))## reshape th train data from the CSV file
