	...
	for name in sorted(dirnames):
		...
		for files in file_name:
			...
			if os.path.isfile(''.join([str(sys.argv[1]),'/',name,'/',files])):
				...
				if "train" in files and os.path.isdir(''.join([str(sys.argv[1]),'/',name,'/##folder_name##']))==0: ##define the result folder per subject ONLY USE THIS TO TEST THE NEXT SUBJECT AND DEFINE THE RECTANGULAR KERNEL SIZE TEST.
					...
					subprocess.call('python CNN_launching_classifier_emotions_innvestigate.py "%s" "%s" "%s"'% (str1,str2,str(sys.argv[2])), shell=True)
