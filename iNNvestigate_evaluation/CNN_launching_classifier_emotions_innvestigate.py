...
str2=str(sys.argv[2])
strn1=str1.split('_')
strn2=strn1[8].split('.') ## change it for 10 or 9 depending if the folder name has a _ please check the length of your pathname and the number of _s it contains 

csvFile1 = str(sys.argv[1]) # training and test files taking per subject specifiy these files in the LOTO CV in an upper level
csvFile2 = str(sys.argv[2])
csv_method="/path_preffix/"+strn1[7]+"/method_"+strn2[0]+"_"+str(sys.argv[4])+".txt" #path_for_method_mask_provided_by_iNNvestigate ## refer to some mask examples provided by the 
examples folder
...
num_colsm = method_file.shape[1] #test_data_file.shape[1] # Length of Data Vecto
...
pos_f=0.5*(data_m.shape[0]*data_m.shape[1]) ## change this parameter between 0-1 and check the mask and corresponding performance output a different r value produces
...
def conv_net(x_dict, n_classes, dropout, reuse, is_training):
    ...
    with tf.variable_scope('ConvNet', reuse=reuse):
        ...
        conv1 = tf.layers.conv2d(x, filters=32, kernel_size=[100,10], activation=tf.nn.relu, name='conv1d')
        ...
        conv2 = tf.layers.conv2d(conv1, filters=64, kernel_size=[20,5], activation=tf.nn.relu, name='conv2d')
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        conv2 = tf.layers.max_pooling2d(conv2, pool_size=[2,2], strides=2)
        ## add the normalization if you like or not is not necessary for the final performance achieving
        tf.layers.LayerNormalization(axis=1 , center=True , scale=True)
        # Convolution Layer with 128 filters and a kernel size of 5x2
        conv3 = tf.layers.conv2d(conv2, filters=128, kernel_size=[10,2], activation=tf.nn.relu, name='conv3d')
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
...
def model_fn(features, labels, mode):
    ...
    acc_op = tf.metrics.accuracy(labels=labels, predictions=pred_classes, name="labels")
...
label_train=label_train.astype('float32',casting='same_kind')-1 ## labels should be defined between 0-3 
data_test=data_test.astype('float32',casting='same_kind')
label_test=label_test.astype('float32',casting='same_kind')-1

# Build the Estimator
model = tf.estimator.Estimator(model_fn,cwd+'/folder_check_points_innvestigate')
